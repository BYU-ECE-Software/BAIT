    ...
    <Modal bind:open={regFormModal} size="xs" autoclose={false} class="w-full">
        <form onsubmit={handleRegister} class="flex flex-col space-y-6">
            ...
            <Label class="space-y-2">
                <span>Full Name</span>
                <Input type="text" bind:value={fullName} placeholder="John Doe" required />
            </Label>
            <Label class="space-y-2">
                <span>Email</span>
            ...
            <div class="flex items-start">
                <Checkbox>I agree to the <a href="/" class="text-primary-700 hover:underline dark:text-primary-500">terms and conditions</a></Checkbox>
            </div>
            <Button type="submit" class="w-full">Create account</Button>
            <div class="text-sm font-medium text-gray-500 dark:text-gray-300">
                Already have an account? <a href="/" onclick={(e) => { e.preventDefault(); onSwitchToSignIn();}} class="text-primary-700 hover:underline dark:text-primary-500">Sign in</a>
            </div>
        </form>
    </Modal>
