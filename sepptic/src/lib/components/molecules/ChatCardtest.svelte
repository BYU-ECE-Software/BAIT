<script lang="ts">
  ...
  onMount(async () => {
    console.log('🔍 fetching history for', { campaignId, characterId, sentfrom });
    try {
      const res = await fetch(
        `/api/message?campaignId=${campaignId}&characterId=${characterId}&sentfrom=${encodeURIComponent(sentfrom)}`,
        { credentials: 'include' }
      );
